...
public class MaintenancePlan
{
    ...
    public int? ToleranceDays { get; set; } // Tolerancia permitida en días
    public string? Responsible { get; set; } // Interno / Externo (nombre de empresa o técnico)
    public bool RequiresStop { get; set; } = false; // Requiere parada del equipo
    public bool RequiresVerification { get; set; } = false; // Requiere verificación posterior
}
...
public class MaintenanceEvent
{
    ...
    public string? PlanName { get; set; } // Tipo o nombre de mantenimiento
    public DateTime? ScheduledDate { get; set; } // Fecha prevista
    public DateTime PerformedAt { get; set; } // Fecha realizada
    public Guid? PerformedByUserId { get; set; }
    public string? PerformedByUserName { get; set; }
    public MaintenanceEventType EventType { get; set; } = MaintenanceEventType.PREVENTIVE;
    public bool IsInternal { get; set; } = true; // Realizado por personal interno o técnico externo
    public string? ActivitiesPerformed { get; set; } // Actividades realizadas
    public string? Outcome { get; set; } // Resultado: Conforme, No conforme, Conforme con observaciones
    public bool HasDeviation { get; set; } = false; // Desviación respecto a la fecha prevista
    public string? DeviationReason { get; set; } // Motivo de desviación
    public bool RequiresAdditionalAction { get; set; } = false; // Requiere acción adicional
    public bool RequiresVerification { get; set; } = false; // Requiere verificación posterior
    public bool VerificationPerformed { get; set; } = false; // Verificación realizada
    public EquipmentStatus? EndStatus { get; set; } // Estado del equipo tras el mantenimiento
    public string? Notes { get; set; }
    public Guid? EvidenceDocId { get; set; }
    public string? CertificatePath { get; set; } // Enlace al certificado
    public decimal? Cost { get; set; }
    public bool IsEfficiencyCheck { get; set; } // Para verificaciones funcionales asociadas
...
public class EquipmentHistory
{
    ...
    public string ActionType { get; set; } = string.Empty; // STATUS_CHANGE, QC_RECORD, CALIBRATION, INCIDENT, REPAIR, ACCEPTANCE, etc.
